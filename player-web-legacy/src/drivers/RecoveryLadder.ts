...
export class RecoveryLadder {
    ...
    private pendingReload: {
        ...
        attempt: number;
        /** The runtime was suspended while this request was outstanding. */
        suspended: boolean;
    } | null = null;
    /** Between {@link noteSuspended} and {@link noteResumed}. */
    ...
    noteResumed(): void {
        ...
        if (this.stopped || !this.pendingReload?.suspended) return;
        this.pendingReload.suspended = false;
        const { reason, attempt } = this.pendingReload;
        this.hooks.requestReload(reason, attempt);
    }

    /**
     * The app went to the background. Only a request outstanding across a
     * suspension may have gone undelivered; one raised while the runtime stayed
     * awake reached the wrapper, and raising it again would rebuild the source
     * a second time.
     */
    noteSuspended(): void {
        this.suspended = true;
        if (this.pendingReload) this.pendingReload.suspended = true;
    }
    ...
    private climb(payload: AdapterErrorPayload, reason: RecoveryReason): void {
        ...
        this.timer = setTimeout(() => {
            ...
            this.pendingReload = { reason, attempt: attempt + 1, suspended: this.suspended };
