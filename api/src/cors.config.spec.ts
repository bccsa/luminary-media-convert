...
function decide(
    ...
    return new Promise((resolve, reject) => {
        ...
        fn(origin, (err, allow) => (err ? reject(err) : resolve(allow === true)));
    });
}
...
describe('CORS — the origin decision', () => {
    it('asks the registry about a browser origin', async () => {
        ...
        await expect(decide(options, 'http://localhost:5199')).resolves.toBe(true);
        await expect(decide(options, 'https://evil.example')).resolves.toBe(false);
    });
    ...
    it('waits on an approval the registry has to ask a human for', async () => {
        ...
        await expect(decide(options, 'https://cms.example.com')).resolves.toBe(true);
        await expect(decide(options, 'https://other.example')).resolves.toBe(false);
    });

    it.each([undefined, 'null'])('allows a request whose origin is %o', async (origin) => {
        ...
        await expect(decide(createCorsOptions(registry), origin)).resolves.toBe(true);
    });
    ...
    it('refuses by withholding the header, never by raising', async () => {
        ...
        await expect(decide(createCorsOptions(registry), 'https://x.example'))
            .resolves.toBe(false);
    });

    it('treats a rejected approval as a refusal', async () => {
        const registry = registryAllowing(() => Promise.reject(new Error('dialog failed')));
