...
export class CmsHealthResponseDto {
    @ApiProperty({ description: 'Always "ok" when the local encoder is reachable.', example: 'ok' })
    @Expose()
    status: string;

    @ApiProperty({ description: 'Version of the local encoder.', example: '0.0.1' })
    @Expose()
    apiVersion: string;
}
...
export class CmsSessionResponseDto {
    ...
    @ApiProperty({
        description:
            ...
            'the query string.',
        example: 'http://127.0.0.1:31711/api/sessions/a1b2c3d4/events?token=read_f8e7',
    })
    @Expose()
    eventsUrl: string;
