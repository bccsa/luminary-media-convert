...
<template>
    <div ref="rootEl" class="relative shrink-0">
        <button
            type="button"
            class="flex h-9 w-9 cursor-pointer items-center justify-center rounded-full border border-zinc-300 bg-zinc-100 text-xs font-semibold text-zinc-800 shadow-sm ring-zinc-900/5 transition-colors hover:bg-zinc-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/50 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100 dark:hover:bg-zinc-700 dark:ring-white/10"
            :aria-expanded="open"
            aria-haspopup="true"
        ...
        >
            <div
                v-if="open"
                class="absolute right-0 top-full z-50 mt-2 w-[min(18rem,calc(100vw-2rem))] origin-top-right rounded-xl border border-zinc-200 bg-white py-1 shadow-xl ring-1 ring-zinc-900/5 dark:border-zinc-700 dark:bg-zinc-900 dark:ring-white/10"
                role="menu"
                aria-label="Account"
                @click.stop
            >
                <div class="border-b border-zinc-100 px-3 py-3 dark:border-zinc-800">
                    <p class="text-xs font-medium text-zinc-500 dark:text-zinc-400">Signed in as</p>
                    <p class="mt-0.5 truncate text-sm font-medium text-zinc-900 dark:text-zinc-100" :title="email">
                        {{ email || '—' }}
                    </p>
                </div>
                ...
                <div class="px-2 py-2" role="none">
                    <p
                        ...
                        class="px-2 pb-1.5 text-[11px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500"
                    >
                        Appearance
                    </p>
                    <div class="space-y-0.5" role="group" aria-labelledby="account-appearance-label">
                        <button
                            ...
                            class="flex w-full cursor-pointer items-center gap-2 rounded-lg px-2 py-2 text-left text-sm text-zinc-800 transition-colors hover:bg-zinc-50 dark:text-zinc-200 dark:hover:bg-zinc-800"
