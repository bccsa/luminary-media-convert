...
<template>
    <div class="space-y-3">
        ...
        <template v-if="isCompleted">
            <div v-if="displayMasterPlaylist" class="rounded-lg border border-slate-200 bg-slate-50/95 p-3 dark:border-slate-700 dark:bg-slate-800/60">
                ...
                <p class="break-all font-mono text-xs leading-snug text-slate-700 dark:text-slate-400">{{ s3Url ?? displayMasterPlaylist }}</p>
            </div>

            <div v-if="isEncrypted && encryptionKeyHex" class="rounded-lg border border-slate-200 bg-slate-50/95 p-3 dark:border-slate-700 dark:bg-slate-800/60">
                <div class="mb-1 flex items-center justify-between gap-2">
                ...
                <code class="block break-all rounded-md bg-slate-100 px-2 py-1.5 font-mono text-[11px] leading-snug text-amber-800 dark:bg-slate-800 dark:text-amber-400">{{ encryptionKeyHex }}</code>
            </div>

            <details
                v-if="displayFiles?.length"
                class="group rounded-lg border border-slate-200 bg-slate-50/95 dark:border-slate-700 dark:bg-slate-800/60"
            >
                <summary
                    class="flex cursor-pointer list-none items-center gap-2 rounded-lg px-3 py-2 text-left [&::-webkit-details-marker]:hidden"
                >
                ...
                <div class="border-t border-slate-200 px-3 pb-3 pt-1.5 dark:border-slate-700">
                        ...
                        <table class="w-full min-w-[28rem] text-left text-xs">
                            <thead>
                                <tr class="border-b border-slate-200 text-slate-500 dark:border-slate-700 dark:text-slate-400">
                                    <th class="pb-1 pr-2 font-medium">File</th>
                                    <th class="pb-1 pr-2 font-medium">Type</th>
                                    <th class="pb-1 pr-2 font-medium">Size</th>
                                    <th class="pb-1 font-medium text-right">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr
