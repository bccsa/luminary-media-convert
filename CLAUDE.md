...
- **`player-core/`** — Framework-agnostic, headless player wrapper: HLS munging pipeline (client-side angle extraction, quality capping, key handling, LMCENC decryption), `PlayerController` state store, coming-soon polling, and the `PlayerAdapter` contract for pluggable engines (Video.js/hls.js today; AVPlayer/ExoPlayer adapters later in a Capacitor shell). Headless in the strict sense — no `Blob`, no `URL.createObjectURL`, no DOM — so a host must supply a `ServeStrategy` (`PlayerControllerOptions.serveStrategy` is required; the web's blob-backed one lives in the player packages). Detection and recovery are the engine's: an adapter spends its own ladder before raising a fatal `error`, and reaches back through exactly one door — `reload-requested` — for the only repair no engine can make, rebuilding a munged source. Published as `@luminary-media-converter/player-core`. See `player-core/src/types.ts` for the full contract and `docs/suspension-safe-playback.md` for why the line falls there.
- **`player-web/`** — **Deprecated**, superseded by `player-web-legacy`, whose `LuminaryPlayer` has the same props, slots and exposed surface. It keeps building only because `app/` still consumes it, and gets no new work; every export of its own is tagged `@deprecated` with its replacement. The hls.js web player: `HlsJsAdapter` (hls.js on a plain `<video>`, in-memory AES key delivery via a custom key loader — no key blob URLs), `LuminaryPlayer.vue`, iOS-style fullscreen controls with orientation lock, and `PlayerMessages` i18n (every user-facing string overridable; scoped slots for full custom UI). It draws no chrome over the picture outside fullscreen: entering is a double-click / double-tap on the video, or `enterFullscreen()` from the host — which is where the button belongs (the encoder puts it beside its angle / audio / quality selectors). Inside fullscreen the controls, exit button included, are the player's. Also carries a copy of `player-web-legacy`'s **chunk-warming loop** (`ChunkPrefetcher` in `adapter/chunkWarming.ts`, driven through the optional `PlayerAdapter.warmChunks` contract — see `docs/chunk-warming.md`) and a dev-only test harness (`npm -w player-web run demo`: plays any master URL + optional key through the real player, with warming console instrumentation). Published as `@luminary-media-converter/player-web`; consumed by `app/`.
- **`player-web-legacy/`** — The maintained web implementation of the player — it supersedes the hls.js `player-web`, despite its name — on Video.js 8 (VHS 3.17.5): `VideoJsAdapter` over `player-core`'s `PlayerAdapter` contract, and a `LuminaryPlayer.vue` with the same props, slots and `defineExpose` surface as `player-web`'s, so an app can swap one for the other. It exists to replicate the [bccsa/luminary](https://github.com/bccsa/luminary) app's video.js skin — video.js owns the whole chrome, in every mode, and `styles.css` repositions its stock components. In-memory `luminary://key` delivery comes from wrapping VHS's per-handler xhr factory (`vhsXhrSeam.ts`, policy in `vhsKeyInterceptor.ts`) rather than from an hls.js key loader; the same seam gives byte-range segment requests a longer timeout (`vhsRequestTimeout.ts` — VHS's default of 1.5 × target duration forces a switch-down on every cold chunk object, and all renditions share the object); stall detection is VHS's own, read off its `usage` events (`vhsStallSignals.ts`); live media playlists are answered on the same seam (`serveLive` mints `luminary://live/<n>`, `vhsLivePlaylistInterceptor.ts` re-reads upstream on every VHS refresh); an optional YouTube mode plays a YouTube URL through `videojs-youtube`, bypassing the LMC pipeline entirely with a null controller. Same dev harness (`npm -w player-web-legacy run demo`). Published as `@luminary-media-converter/player-web-legacy`; consumed by the Luminary app via a git-submodule checkout plus a file-reference install, not by anything in this repo.
...
- **Media playback**: `LuminaryPlayer` from `@luminary-media-converter/player-web` (hls.js on a plain `<video>`, driven by the `player-core` controller) — deprecated; the app has yet to move to `player-web-legacy`. Angle switching, quality selection, audio tracks and encrypted playback all go through the controller; the AES key is fetched masked from `GET /api/sessions/:id/key`, unmasked in memory (`utils/keyMask.ts`) and handed to the player, which serves it to hls.js from memory — no key blob URLs
...
3. Feed the munged playlists to the engine. **`player-core` (`PlayerController` + pipeline) is the reference implementation**, with `player-web-legacy`'s `VideoJsAdapter`/`LuminaryPlayer` as the web engine binding. The media encoder app still consumes the deprecated `player-web`'s `HlsJsAdapter`/`LuminaryPlayer`.
...
- **Every variant in `master.m3u8` declares its audio group's codec in `CODECS`**, not only its own video codec. A player that finds two codecs on one variant and one on another concludes the streams differ and refuses the odd one out — VHS marks it incompatible for good — which silently removed the whole bandwidth-saving tier on ladders split into waves (`MAX_HARDWARE_SESSIONS`): only the first wave carries audio, ffmpeg writes the audio codec only onto that wave's variants, and `mergeWaveMasters` could restore it only for a group some first-wave variant named. It now also takes the encoder's own knowledge (`AudioCodecsFor`): a re-encoded group is `mp4a.40.2` because `aacArgs` forces AAC-LC (`AAC_LC_CODEC_ATTRIBUTE`), and a copy-mode group's attribute is read from the init segment ffmpeg wrote (`aac-codec-attribute.ts`, on the `fmp4-timeline.ts` box walker) — the same bytes ffmpeg's own `CODECS` came from. Sessions encoded before this keep their master and need a re-encode or a one-off edit to regain the low renditions
...
- **The line between `player-core` and an adapter is what must keep working while JavaScript is frozen.** A native engine keeps pulling segments from its own threads while a locked screen suspends the WebView, so anything that acts *during* playback belongs beside the engine. `player-core` keeps munging, quality capping, angle switching, coming-soon polling, chapters/subtitles and the state store — none of which run during playback, or matter if they pause. Adapters own stall *detection* (VHS's `PlaybackWatcher`, hls.js's gap controller, the native players all watch their own buffer; a wrapper timer sampling `currentTime` cannot tell a slow request from a wedged decoder, and the one `player-core` used to run made stalls worse — it is gone, surfaced instead as the `stalled` adapter event), the recovery ladder (engine primitive → bounded `reattach()` with backoff on a MONOTONIC clock → `reload-requested`) and chunk warming. `docs/suspension-safe-playback.md` is the normative spec a native adapter implements against and `player-web-legacy/src/drivers/` is what it ports; `reattach()` is separate from the re-munge because the munge is deterministic for an unchanged selection (a reload produced byte-identical playlists behind fresh URLs, so only the engine being rebuilt ever mattered), which is what lets a backgrounded player recover without waking JavaScript. Implementing apps talk to `PlayerController` only. There is no munge fast path: every source — encrypted or not — fetches and rewrites its media playlists, so behavior never depends on what the source happens to be
...
- **HLS output is always square-pixel.** A coded frame size says nothing about the shape of the picture in it: a 720x576 PAL broadcast carrying 16:9 stores non-square samples (SAR 64:45) and is shown 1024x576. `ProbeService` reads `sample_aspect_ratio` and publishes `displayWidth`/`displayHeight` alongside the coded ones; the ladder, the preview and the storyboard are all built from those, and every scaled rendition is tagged `setsar=1` after its scaler — unconditionally, because `scale`/`scale_vt` correct SAR to preserve display aspect while `scale_cuda`/`vpp_qsv` inherit it, and nothing should depend on which machine ran the encode. The correction never downscales: wide pixels widen, tall pixels heighten. `aspect.ts` (duplicated into `encode-config/` for the same reason `copyMode.ts` is) is the only place that answers "what shape is this"
...
- **VOD is read once, and that is what makes it suspension-safe**: the wrapper munges at load and hands finished text to the `ServeStrategy`, so JavaScript is off the request path for the rest of playback. A LIVE playlist (no `#EXT-X-ENDLIST`) breaks that — re-fetched, re-decrypted and re-rewritten every target-duration — so the refresh belongs to the serving layer, which declares the capability by implementing `serveLive(spec)`. Its absence makes the pipeline refuse a live source (`PlayerError{ code: 'live-unsupported' }`) rather than serve a snapshot frozen at its first read. `LivePlaylistSpec` is plain data so a native resolver does the whole job unaided, and `resolveLivePlaylist` (fetch → LMCENC sniff → `rewriteMediaPlaylist`, one read per engine request, no timer — the engine's own refresh drives it) is the step it ports. `player-web-legacy` implements it: `BlobServeStrategy.serveLive` mints `luminary://live/<n>` and `vhsLivePlaylistInterceptor.ts` answers VHS's requests for it on the xhr seam. `player-web` (hls.js, deprecated) does not, and refuses live. Nothing here *produces* live output (`-hls_playlist_type vod`); live sources are third-party
- `rewriteMediaPlaylist` is deliberately **string work, not a model round-trip**: it makes two edits (substitute the key URI, absolutize URIs), it is strictly more preserving that way (a line it does not recognise is untouched rather than rebuilt from a model), and it is the one rewrite on live's per-request path, so it is what gets ported to Swift and Kotlin. `hls-core`'s lossless model stays where it earns its keep — the load-time master munge, and the API's own playlist building
